...
const draft = {
  id: 6,
  recipe_name: "Actisoft Menekşe 1,5 L x 4",
  marketplace: "TRENDYOL",
  provider_mode: "MOCK_DRAFT",
  workflow_status: "AI_DRAFT",
  safety_errors: [],
  safety_warnings: ["BRAND_NOT_IN_TITLE"],
  current_content: { title: "Eski başlık", description: "Eski açıklama" },
  proposed_content: {
    title: "Actisoft Menekşe 1,5 L x 4",
    description: "Doğrulanmış içerik",
    searchTerms: "actisoft menekşe",
    visualBriefs: [{ type: "MAIN", brief: "4 gerçek paket" }],
  },
  diff_json: [
    {
      field: "title",
      current: "Eski başlık",
      proposed: "Actisoft Menekşe 1,5 L x 4",
    },
  ],
  snapshots: [
    {
      ...
      content_json: { title: "Eski başlık" },
    },
  ],
};
...
const health = {
  ...
  seller_listing_barcode: "TY-MENEKSE",
  quality_score: 58,
  confidence: "HIGH",
  publication_state: "PUBLISHED",
  summary: "2 doğrulanabilir iyileştirme alanı bulundu",
  checks_json: [
    {
      code: "PACK_COUNT_MISSING",
      label: "Paket adedi",
      status: "ISSUE",
      penalty: 12,
      evidence: { expected: 4 },
      recommendation: "Paket adedini başlığa ekleyin",
      expectedImpact: "İçerik açıklığını artırabilir",
      kpi: "yanlış beklenti/iade",
    },
  ],
...
describe("İçerik Stüdyosu", () => {
  ...
  test("taslak üretimi reçete ve pazaryeriyle açık onay ister", async () => {
    ...
    await user.click(
      screen.getAllByRole("button", { name: "Taslak üret" }).at(-1),
    );
...
describe("Listing Sağlığı", () => {
  test("sorun kanıtını, öneriyi ve ölçülecek KPI'yı açıklar", async () => {
    ...
    expect(screen.getByText("Dönüşüm oranı")).toBeVisible();
  });
});
