...
const opportunity = {
  ...
  signal_breakdown: [
    {
      key: "supplier_freshness",
      label: "Tedarikçi fiyat güncelliği",
      source: "SUPPLIER_POOL",
      value: 2,
      contribution: 10,
    },
  ],
...
describe("Ürün Fırsatları", () => {
  ...
  test("puan katkısı, eksik veri ve reçete bileşenini drawerda açıklar", async () => {
    ...
    await user.click(
      await screen.findByText("Menekşe Çamaşır Yumuşatıcısı 1,5 L x 3"),
    );
  ...
  test("fırsat üretimi açık onayla ve seçili hedef pazaryeriyle çalışır", async () => {
    ...
    render(<OpportunityCenter marketplace="TRENDYOL" notify={vi.fn()} />);
    await user.click(
      await screen.findByRole("button", { name: "Fırsatları üret" }),
    );
    await user.click(
      screen.getAllByRole("button", { name: "Fırsatları üret" }).at(-1),
    );
  ...
  test("ret nedeni açık onayla immutable karar geçmişine gönderilir", async () => {
    post.mockResolvedValue({
      data: {
        ...
        rejection_reason: "Paketleme uygun değil",
      },
    });
    const user = userEvent.setup();
    render(<OpportunityCenter marketplace="TRENDYOL" notify={vi.fn()} />);
    ...
    await user.type(
      screen.getByLabelText("Ret nedeni"),
      "Paketleme uygun değil",
    );
