...
function catalogState(
  ...
  if (match) return { status: "MATCH_REVIEW_REQUIRED", barcodeRequired: false };
  if (!integration?.enabled)
    return { status: "MARKETPLACE_DISABLED", barcodeRequired: false };
  if (!integration?.credentials_configured)
    return {
      status: "MARKETPLACE_CREDENTIALS_MISSING",
...
class OpportunityService {
  ...
  async economicsForRecipe(recipe, targetMarketplace, marketRow) {
    const sourceMarketplace =
      targetMarketplace === "TRENDYOL" ? "HEPSIBURADA" : "TRENDYOL";
    try {
  ...
  async generate(input = {}, actor) {
    if (input.confirmation !== "FIRSATLARI_URET")
      throw new AppError(
        "Fırsat üretimi için açık onay gerekli",
        409,
        "OPPORTUNITY_GENERATION_CONFIRMATION_REQUIRED",
      );
    const targetMarketplace = String(
      input.targetMarketplace || "TRENDYOL",
    ).toUpperCase();
    ...
    const marketByRecipe = new Map(
      data.marketRows.map((item) => [item.recipe_id, item]),
    );
    const salesByRecipe = new Map(
      data.sales.map((item) => [
        item.recipe_id,
        Number(item.family_sales || 0),
      ]),
    );
    const existingFingerprints = new Set(
      data.recipes.map((item) => item.bundle_fingerprint),
    );
    ...
    const physical = data.physical.map((item) => ({
      ...
      supplierFreshnessDays: ageInDays(
        item.source_checked_at || item.cost_updated_at,
      ),
      stockAvailable: true,
    }));
    const opportunities = [];
    ...
    for (const recipe of recipes.filter((item) => item.status === "APPROVED")) {
      ...
      const gap =
        Number(market?.buybox_price || 0) -
        Number(market?.calculated_min_price || 0);
      const profitableRankGap =
        market && gap > 0 && Number(market.rank || 99) > 1;
      const competitorCount = [
        market?.buybox_price,
        market?.second_price,
        market?.third_price,
      ].filter((value) => Number(value) > 0).length;
      const lowCompetitionGap =
        market && competitorCount > 0 && competitorCount <= 2 && gap > 0;
      const currentPrice = Number(market?.my_price || market?.sale_price || 0);
      const minimumPrice = Number(market?.calculated_min_price || 0);
      const highMarginVariant =
        market &&
        currentPrice > 0 &&
        minimumPrice > 0 &&
        currentPrice / minimumPrice >= 1.25;
      if (
        !missingListing &&
        !profitableRankGap &&
        !lowCompetitionGap &&
      ...
      const economics = await this.economicsForRecipe(
        recipe,
        targetMarketplace,
        market,
      );
      const score = scoreOpportunity({
        ...
        shippingRatio:
          economics.proposedPrice > 0
            ? economics.shippingCost / economics.proposedPrice
            : null,
        commissionRate: economics.commissionRate || market?.commission_rate,
      });
      if (missingListing)
        add({
          opportunityType: "MISSING_MARKETPLACE",
          recipeId: recipe.id,
          proposedRecipe: {
          ...
          bundleFingerprint: recipe.bundle_fingerprint,
          score: score.score,
      ...
      if (lowCompetitionGap)
        add({
          opportunityType: "LOW_COMPETITION_GAP",
          recipeId: recipe.id,
          proposedRecipe: {},
          bundleFingerprint: recipe.bundle_fingerprint,
          score: score.score,
      ...
      if (highMarginVariant)
        add({
          opportunityType: "HIGH_MARGIN_VARIANT",
          recipeId: recipe.id,
          proposedRecipe: {},
          bundleFingerprint: recipe.bundle_fingerprint,
          score: score.score,
    ...
    const singleFingerprints = new Set(
      physical.map((product) =>
        bundleFingerprint([
          { costItemCode: product.costItemCode, quantity: 1 },
        ]),
      ),
    );
    for (const product of physical.filter(
      (item) =>
        !existingFingerprints.has(
          bundleFingerprint([{ costItemCode: item.costItemCode, quantity: 1 }]),
        ),
    )) {
      ...
      const score = scoreOpportunity({
        supplierFreshnessDays: product.supplierFreshnessDays,
        stockAvailable: true,
        missingPack: true,
      });
      add({
        opportunityType: "MISSING_SINGLE",
        proposedRecipe: {
          recipeName: product.productName,
          recipeType: "SINGLE",
          components: [{ costItemCode: product.costItemCode, quantity: 1 }],
          totalCostMinor: product.unitCostMinor,
          fractionalDesi: product.unitDesi,
        ...
        signals: score.signals,
        economics: {
          productCost: product.unitCostMinor / 100,
          desi: Math.ceil(product.unitDesi),
          dataStatus: "PARTIAL",
          missing: ["category", "commission", "shipping", "buybox"],
        },
        catalogStatus: "SEARCH_REQUIRED",
        listingBarcodeRequired: false,
        dataQuality: { missing: score.missing, economics: "PARTIAL" },
        generationReason:
          "Tedarikçi maliyet havuzunda var, PIM'de tekli reçete yok",
      });
    }
    const bundleInput = {
      existingFingerprints: [
        ...new Set([...existingFingerprints, ...singleFingerprints]),
      ],
      maxCandidates: Number(input.maxBundleCandidates || 100),
    };
    ...
    const mixedCandidates = generateMixedBundleCandidates(physical, {
      ...
      existingFingerprints: [
        ...existingFingerprints,
        ...packCandidates.map((item) => item.bundleFingerprint),
      ],
    });
    for (const [type, candidates] of [
      ["MISSING_PACK_SIZE", packCandidates],
      ["MIXED_BUNDLE", mixedCandidates],
    ])
      for (const candidate of candidates) {
        const freshness = Math.max(
          ...candidate.components.map(
            (component) =>
              physical.find(
                (item) => item.costItemCode === component.costItemCode,
              )?.supplierFreshnessDays || 999,
          ),
        );
        const score = scoreOpportunity({
          supplierFreshnessDays: freshness,
          stockAvailable: true,
          missingPack: true,
        });
        add({
          opportunityType: type,
          proposedRecipe: candidate,
          bundleFingerprint: candidate.bundleFingerprint,
          score: score.score,
          confidence: score.confidence,
          signals: score.signals,
          economics: {
            productCost: candidate.totalCostMinor / 100,
            desi: candidate.finalDesi,
            dataStatus: "PARTIAL",
            missing: ["category", "commission", "shipping", "buybox"],
          },
          catalogStatus: "SEARCH_REQUIRED",
          ...
          generationReason:
            type === "MIXED_BUNDLE"
              ? "Aynı ürün ailesinde eksik karma paket"
              : "Mevcut ürün ailesinde eksik paket adedi",
        });
      }
    ...
    return {
      ...
      byType: saved.reduce(
        (result, item) => ({
          ...result,
          [item.opportunity_type]: (result[item.opportunity_type] || 0) + 1,
        }),
        {},
      ),
  ...
  async approve(id, actor, confirmation) {
    if (confirmation !== "FIRSAT_RECETESINI_ONAYLA")
      throw new AppError(
        "Fırsat reçetesi için açık onay gerekli",
        409,
        "OPPORTUNITY_APPROVAL_CONFIRMATION_REQUIRED",
      );
    ...
    return this.repository.transition(id, {
      status: "RECIPE_APPROVED",
      recipeId,
      actor,
      eventType: "RECIPE_APPROVED",
      snapshot: {
        score: opportunity.score,
        proposedRecipe: opportunity.proposed_recipe,
      },
    });
  }
  ...
  async reject(id, actor, input = {}) {
    if (
      input.confirmation !== "FIRSATI_REDDET" ||
      !String(input.reason || "").trim()
    )
      throw new AppError(
        "Ret için açık onay ve neden zorunlu",
        409,
        "OPPORTUNITY_REJECTION_CONFIRMATION_REQUIRED",
      );
    const opportunity = await this.repository.get(id);
    if (!opportunity)
      throw new AppError("Fırsat bulunamadı", 404, "OPPORTUNITY_NOT_FOUND");
    return this.repository.transition(id, {
      ...
      snapshot: {
        score: opportunity.score,
        recipe: opportunity.proposed_recipe,
  ...
  async searchCatalog(id, actor) {
    ...
    const status = candidates.length
      ...
      : opportunity.workflow_status === "RECIPE_APPROVED"
        ? "CATALOG_SEARCHED"
