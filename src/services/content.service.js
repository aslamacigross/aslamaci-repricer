...
class ContentService {
  ...
  listDrafts(filters) { return this.repository.listDrafts(filters); }
  getDraft(id) { return this.repository.getDraft(id); }
  listHealth(filters) { return this.repository.listHealth(filters); }
  getHealth(id) { return this.repository.getHealth(id); }

  async generate(input = {}, actor) {
    if (input.confirmation !== "ICERIK_TASLAGI_URET")
      throw new AppError("İçerik taslağı için açık onay gerekli", 409, "CONTENT_DRAFT_CONFIRMATION_REQUIRED");
    ...
    if (listing && listing.marketplace !== marketplace)
      throw new AppError("Listing pazaryeri uyuşmuyor", 409, "MARKETPLACE_ISOLATION_VIOLATION");
    ...
    const draft = await this.repository.saveDraft({
      ...
      sourceProvenance: [
        ...
        ...(recipe.components || []).map((item) => ({ type: "COST_ITEM", id: item.cost_item_code })),
        ...(listing ? [{ type: "MARKETPLACE_LISTING", id: listing.id, marketplace }] : []),
      ],
  ...
  async update(id, input = {}, actor) {
    ...
    if (!draft) throw new AppError("İçerik taslağı bulunamadı", 404, "CONTENT_DRAFT_NOT_FOUND");
    if (["MARKETPLACE_SUBMITTED", "VERIFIED"].includes(draft.workflow_status))
      throw new AppError("Gönderilmiş içerik taslağı düzenlenemez", 409, "CONTENT_DRAFT_LOCKED");
    const proposed = { ...draft.proposed_content, ...(input.proposedContent || {}) };
    proposed.metadata = draft.proposed_content?.metadata || draft.source_facts?.metadata || {
  ...
  async approve(id, actor, confirmation) {
    if (confirmation !== "ICERIGI_ONAYLA")
      throw new AppError("İçerik onayı için açık onay gerekli", 409, "CONTENT_APPROVAL_CONFIRMATION_REQUIRED");
    const draft = await this.repository.getDraft(id);
    if (!draft) throw new AppError("İçerik taslağı bulunamadı", 404, "CONTENT_DRAFT_NOT_FOUND");
    const validation = validateContent(draft.proposed_content, draft.source_facts);
    if (validation.errors.length)
      throw new AppError("İçerik güvenlik doğrulamasından geçmedi", 409, "CONTENT_SAFETY_FAILED", validation);
    return this.repository.approveDraft(id, actor, contentChecksum(draft.proposed_content));
  }

  async publishDryRun(id, actor, confirmation) {
    if (confirmation !== "ICERIK_DRY_RUN_ONAYLA")
      throw new AppError("İçerik dry-run için açık onay gerekli", 409, "CONTENT_DRY_RUN_CONFIRMATION_REQUIRED");
    const draft = await this.repository.getDraft(id);
    if (!draft) throw new AppError("İçerik taslağı bulunamadı", 404, "CONTENT_DRAFT_NOT_FOUND");
    if (draft.workflow_status !== "APPROVED")
      throw new AppError("Önce içerik insan onayı almalı", 409, "CONTENT_NOT_APPROVED");
    ...
    const blockers = [
      ...
      !integration?.capabilities?.supportsContentUpdate && "CAPABILITY_NOT_SUPPORTED",
    ...
    return {
      ...
      rollbackSnapshot: draft.snapshots?.find((item) => item.snapshot_type === "CURRENT") || null,
      mutationPerformed: false,
    };
  }
  ...
  async rollbackPreview(id, input = {}) {
    if (input.confirmation !== "ROLLBACK_ONIZLE")
      throw new AppError("Rollback önizlemesi için açık onay gerekli", 409, "ROLLBACK_PREVIEW_CONFIRMATION_REQUIRED");
    const draft = await this.repository.getDraft(id);
    if (!draft) throw new AppError("İçerik taslağı bulunamadı", 404, "CONTENT_DRAFT_NOT_FOUND");
    ...
    if (!snapshot || Number(snapshot.content_draft_id) !== Number(draft.id))
      throw new AppError("Rollback snapshot bulunamadı", 404, "ROLLBACK_SNAPSHOT_NOT_FOUND");
    const validation = validateContent(snapshot.content_json, draft.source_facts);
    return {
      snapshot,
  ...
  async scanHealth(input = {}, actor) {
    if (input.confirmation !== "LISTING_SAGLIGINI_TARA")
      throw new AppError("Listing sağlık taraması için açık onay gerekli", 409, "LISTING_HEALTH_CONFIRMATION_REQUIRED");
    const marketplace = String(input.marketplace || "TRENDYOL").toUpperCase();
