...
class ActionService {
  ...
  async apply(id, actor) {
    const preparation = await this.withTransaction(async (client) => {
      ...
      const open = await this.actions.findOpen(locked.barcode, client);
      ...
      proposal.expectedProfit = Number(locked.expected_profit);
      proposal.expectedMargin = Number(locked.expected_margin);
