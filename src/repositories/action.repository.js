class ActionRepository {
  ...
  async create(input, client = this.db) {
    return (
      await client.query(
        ...
        expected_margin,safety_checks,expires_at,net_profit_before
  ...
  async pendingOutcomes(elapsedMinutes) {
    return (
      ...
      AND NOT EXISTS(SELECT 1 FROM repricer_outcomes ro WHERE ro.action_id=ra.id AND ro.elapsed_minutes=$1)`,
        [elapsedMinutes],
      )
    ).rows;
  }
  ...
  async recordOutcome(action, outcome) {
    return (
      await this.db.query(
        `INSERT INTO repricer_outcomes(action_id,rank_before,rank_after,buybox_before,buybox_after,buybox_won,buybox_lost,profit_before,expected_profit,elapsed_minutes,outcome)
  ...
  async todayStats(barcode, client = this.db) {
    return (
      await client.query(
       ...
       FROM repricer_actions WHERE barcode=$1 AND created_at>=CURRENT_DATE AND status IN('SENT','SUCCESS','AWAITING_RESULT')`,
        [barcode],
      )
    ).rows[0];
  }
  ...
  async applyLearningOutcome(action, outcome) {
    const success = outcome.buyboxWon;
    const current =
      (
        await this.db.query(
    ...
    } else if (Number(action.proposed_price) < Number(action.old_price)) {
      failed++;
      consecutive++;
      if (consecutive <= 3)
        learned = Math.min(
