class JobRepository {
  ...
  async runs(limit=100) {
    return (await this.db.query("SELECT * FROM job_runs ORDER BY started_at DESC LIMIT $1",[Math.min(Number(limit)||100,500)])).rows;
  }

  async start(name, client=this.db) {
    const job=(await client.query("SELECT * FROM jobs WHERE name=$1",[name])).rows[0];
    return (await client.query(
      `INSERT INTO job_runs(job_id,job_name,status) VALUES($1,$2,'RUNNING') RETURNING *`,[job?.id||null,name]
    )).rows[0];
  }
  ...
  async finish(id, result, client=this.db) {
    return (await client.query(
      ...
      [id,result.status,result.processed||0,result.successful||0,result.failed||0,result.error||null,JSON.stringify(result.metadata||{})]
    )).rows[0];
  }
}
