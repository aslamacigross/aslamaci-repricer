...
} = require("../../src/services/content-provider");
  ...
  const service = new ContentService({
    repository: {
      ...
      saveDraft: async (input) => {
        saved = input;
        return { id: 2, provider_mode: input.providerMode };
      },
    },
  ...
  const service = new ContentService({
    ...
    marketplaceRegistry: {
      get: async () => ({
        ...
        capabilities: { supportsContentUpdate: true },
      }),
      execute: async () => {
        executeCalled = true;
      },
