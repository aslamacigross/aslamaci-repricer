...
test("fırsat puanı sinyal katkılarını açıklanabilir üretir", () => {
  ...
  assert.equal(result.signals.every((item) => item.key && item.source), true);
});
...
test("Menekşe 1,5 L için eksik 2/3/4/6 paketleri güvenli sınırda üretir", () => {
  const existing = bundleFingerprint([{ costItemCode: menekse.costItemCode, quantity: 4 }]);
  const candidates = generatePackCandidates([menekse], { existingFingerprints: [existing] });
  assert.deepEqual(candidates.map((item) => item.components[0].quantity), [2, 3, 6]);
  assert.equal(candidates.find((item) => item.components[0].quantity === 3).finalDesi, 5);
});
...
test("Menekşe ve Çiçek Rüyası karma paketleri ters sırada duplicate üretmez", () => {
  ...
  assert.deepEqual(candidates.map((item) => item.components.map((part) => part.quantity)), [[1, 1], [2, 1], [2, 2], [3, 3], [4, 2]]);
  assert.equal(new Set(candidates.map((item) => item.bundleFingerprint)).size, candidates.length);
  assert.equal(candidates.every((item) => item.recipeType === "MIXED_BUNDLE"), true);
});
...
test("bundle üreticisi maksimum adet, desi ve aday sınırını aşmaz", () => {
  ...
  assert.equal(candidates[0].components.reduce((sum, item) => sum + item.quantity, 0), 2);
});
