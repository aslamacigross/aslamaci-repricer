...
test("yanlış paket adedi ve uydurma sağlık iddiası içerik onayını engeller", () => {
  ...
  const invalid = {
    ...
    bulletPoints: [], visualBriefs: [], metadata: { packageCount: 2 },
  };
  const result = validateContent(invalid, facts);
  assert.ok(result.errors.includes("PACKAGE_COUNT_MISMATCH"));
  assert.ok(result.errors.some((item) => item.startsWith("UNSUPPORTED_CLAIM:")));
});

test("içerik diff mevcut ve önerilen değerleri alan bazında ayırır", () => {
  const result = contentDiff({ title: "Eski", images: ["a"] }, { title: "Yeni", images: ["a"] });
  assert.deepEqual(result, [{ field: "title", current: "Eski", proposed: "Yeni" }]);
});
...
test("listing sağlık puanı sorunu, kanıtı, öneriyi ve ölçülecek KPI'yı açıklar", () => {
  const result = assessListingHealth({
    ...
    description: "Kısa",
    attributes: {}, images: [], video: null, stock: 0,
    sale_price_minor: 40000, buybox_price_minor: 30000,
  }, menekseRecipe(4));
