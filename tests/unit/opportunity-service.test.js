  ...
  const service = new OpportunityService({
    ...
    pim: {
      createRecipe: async (input) => {
        ...
        return { id: 22, status: "DRAFT" };
      },
      getRecipe: async () => ({ id: 22, status: "DRAFT" }),
    },
    publication: {
      approveRecipe: async (...args) => {
...
test("katalog capability yokluğu kontrollü durum olarak kaydedilir", async () => {
  ...
  const service = new OpportunityService({
    repository: {
      ...
      recordCatalogSearch: async (_id, input) => {
        records.push(input);
        return { catalog_status: input.catalogStatus };
      },
    },
    pim: {},
    publication: {},
    marketplaceRegistry: {
      execute: async () => ({
        ...
        message: "Kimlik bilgileri bekleniyor",
      }),
    },
  });
  const result = await service.searchCatalog(5, "admin");
  assert.equal(
    result.opportunity.catalog_status,
  ...
  const service = new OpportunityService({
    repository: {
      generationInputs: async () => ({
        physical: [],
        components: [
          { recipe_id: 7, cost_item_code: "MENEKSE_1500", quantity: 2 },
        ],
        recipes: [
          {
            id: 7,
    ...
    marketplaceRegistry: {
      get: async () => ({
        ...
        capabilities: { supportsCatalogSearch: true },
      }),
    },
  });
  const result = await service.generate(
    { confirmation: "FIRSATLARI_URET", targetMarketplace: "TRENDYOL" },
    "admin",
  );
  assert.equal(result.byType.LOW_COMPETITION_GAP, 1);
  assert.equal(result.byType.HIGH_MARGIN_VARIANT, 1);
  assert.equal(
    generated.some((item) => item.opportunityType === "PROFITABLE_BUYBOX_GAP"),
    false,
  );
});
