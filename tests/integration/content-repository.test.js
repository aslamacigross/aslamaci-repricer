...
const {
  ContentRepository,
} = require("../../src/repositories/content.repository");
...
test("içerik taslağı, diff ve rollback snapshotı transaction içinde korunur", async () => {
  ...
  const draft = await repository.saveDraft({
    ...
    providerMode: "MOCK_DRAFT",
    sourceFacts: { packageCount: 4 },
    currentContent: { title: "Eski başlık" },
    proposedContent: { title: "Yeni başlık" },
    diff: [{ field: "title" }],
    currentChecksum: "old",
    proposedChecksum: "new",
  });
  assert.equal(draft.snapshots.length, 2);
  const updated = await repository.updateDraft(draft.id, {
    proposedContent: { title: "Düzenlenen başlık" },
    diff: [{ field: "title" }],
    safetyErrors: [],
    safetyWarnings: [],
    checksum: "edited",
  ...
  assert.equal(
    approved.snapshots.some((item) => item.snapshot_type === "APPROVED"),
    true,
  );
  await db.end();
});
