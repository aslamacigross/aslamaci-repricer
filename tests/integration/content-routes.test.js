...
function appWith(content) {
  ...
  app.use((req, _res, next) => { req.user = { username: "admin" }; req.id = "content-test"; next(); });
  app.use("/api", contentRoutes({ content, audit: { record: async () => {} } }));
...
test("içerik taslağı endpointi açık onayı ve mutation false sonucunu taşır", async () => {
  ...
  const app = appWith({
    generate: async (...args) => {
      calls.push(args);
      return { draft: { id: 3, provider_mode: "MOCK_DRAFT" }, mutationPerformed: false };
    },
  });
  const response = await request(app).post("/api/content-drafts/generate").send({
...
test("listing sağlık taraması pazaryeri ve onayı servise iletir", async () => {
  const calls = [];
  const app = appWith({
    scanHealth: async (...args) => { calls.push(args); return { processed: 4, mutationPerformed: false }; },
  });
  ...
  assert.equal(response.status, 201);
  assert.deepEqual(calls[0], [{ marketplace: "HEPSIBURADA", confirmation: "LISTING_SAGLIGINI_TARA" }, "admin"]);
});
